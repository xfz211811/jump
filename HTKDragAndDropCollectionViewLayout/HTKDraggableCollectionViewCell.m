...
@property (nonatomic, readwrite, getter=isEditing, assign) BOOL editing;
@end
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _deleteBtn.frame = CGRectMake(-width/3, -width/3, width, width);
        _deleteBtn.hidden = YES;
        _deleteBtn.layer.cornerRadius = width / 2.;
...
{
    _editing = editing;
    [self shakeStatus:_editing];
    self.allowPan = _editing;
    self.deleteBtn.hidden = !_editing;
}

- (void)deleteSelf
{
    if([self.vcDelegate respondsToSelector:@selector(removeCell:)])
        [self.vcDelegate removeCell:self];
}
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if ([self.draggingDelegate respondsToSelector:@selector(userCanEditCell:)]) {
        cellCanDrag = [self.draggingDelegate userCanEditCell:self];
    }
    return cellCanDrag;
}
...
- (HTKDragAndDropCollectionView *)getVC:(UIView *)cell
{
    UIView *superView = cell.superview;
    if([superView isKindOfClass:[UICollectionView class]])
    {
//     
//        UIResponder *nextRep = superView.nextResponder;
//        while(![nextRep isKindOfClass:[HTKDragAndDropCollectionView class]])
//        {
//            nextRep = nextRep.nextResponder;
//        }
        
        return (HTKDragAndDropCollectionView *)superView;
    }
    return [self getVC:superView];;
}

- (void)startEditing
{
    HTKDragAndDropCollectionView *collectionVC = [self getVC:self];
    if(collectionVC)
    {
        collectionVC.editing = YES;
    }
}
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)panGesture {
    ...
    switch (panGesture.state) {
        case UIGestureRecognizerStateBegan: {
            ...
            self.layer.opacity = HTKDraggableCellInitialDragAlphaValue;
            break;
        }
        case UIGestureRecognizerStateChanged: {
        ...
        case UIGestureRecognizerStateEnded: {
         ...
         //   self.editing = YES;
            if ([self.vcDelegate respondsToSelector:@selector(userDidEndDraggingCell:)]) {
                [self.vcDelegate userDidEndDraggingCell:self];
            }
            break;
        }
        default:
